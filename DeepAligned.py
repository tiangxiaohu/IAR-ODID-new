    ...
    def train(self, args, data):
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for batch in train_semi_dataloader:
                ...
                # This part adds hard negative samples
                
                centroids = torch.tensor(km.cluster_centers_).to(self.device)
                # Calculate the similarity between samples and all centroids
                centroids = F.normalize(centroids, 2)
                similarity_to_centroids = torch.matmul(feats, centroids.t())
                # If needed, you can also get the corresponding similarity values
                topk_values = torch.topk(similarity_to_centroids, k=2, dim=1).values
                # Extract the first column of topk_values
                topk_values_col1 = topk_values[:, 0].unsqueeze(1)
                # Append the first column of topk_values to the last column of pos_matrix
                pos_matrix = torch.cat([pos_matrix, topk_values_col1], dim=1)
                # Extract the second column of topk_values, which is the similarity to the second most similar centroid
                topk_values_col2 = topk_values[:, 1].unsqueeze(1)  # Shape becomes [32, 1]
                ...
                if any(ind) is False:
                    ce_loss=0
                    # # loss_w = 0
                    # _,loss_w = self.model(input_ids, segment_ids, input_mask, label_ids, mode="w")
                else:
                    input_ids = input_ids[ind]
                    input_mask = input_mask[ind]
                    segment_ids = segment_ids[ind]
                    label_ids = label_ids[ind]
                    # ce_loss,loss_w = self.model(input_ids, segment_ids, input_mask, label_ids, mode="w")
                    ce_loss = self.model(input_ids, segment_ids, input_mask, label_ids, mode="train_w")
                    # loss_w = self.model(input_ids, segment_ids, input_mask, label_ids, mode="loss_w")
                
                loss=(1 - args.beta) * cl_loss + args.beta * ce_loss 
                # loss= 0.8*cl_loss + 0.1*ce_loss + 0.1*loss_w
                loss.backward()
